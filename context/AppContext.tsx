...
// FIX: Removed duplicate 'Affiliate' type from import.
    ...
    switch (action.type) {
        ...
        case 'ADD_OR_UPDATE_USER': {
            ...
            if (existingUserIndex > -1) {
                ...
                // FIX: Corrected a typo from `existingUser-index` to `existingUserIndex`.
        ...
        case 'AFFILIATE_CONFIRM_INVENTORY_CHANGE': {
            ...
            // FIX: Reset isLowInventoryOrder flag on orders if inventory is now sufficient.
        ...
        case 'CANCEL_INVENTORY_REQUEST': {
            const requestToCancel = state.inventoryChanges.find(c => c.id === action.payload.changeId);
            if (!requestToCancel || requestToCancel.status !== InventoryChangeStatus.Pending) {
                return state;
            }

            const updatedChanges = state.inventoryChanges.filter(
